...
.PARAMETER Hostname
    This is the hostname in the poppet config
.PARAMETER MsiUrl
...
#>
param(
   [String]$HostName = "winbox",
   [string]$MsiUrl = "https://downloads.puppetlabs.com/windows/puppet5/puppet-agent-5.5.8-x64.msi",
   [string]$PuppetVersion = $null
)

if ($PuppetVersion) {
  $MsiUrl = "https://downloads.puppetlabs.com/windows/puppet5/puppet-agent-$($PuppetVersion)-x64.msi"
...
if (!($PuppetInstalled)) {
  ...
  $install_args = @("/qn", "/norestart","/i", $MsiUrl, "PUPPET_AGENT_CERTNAME=$HostName.example.com")
